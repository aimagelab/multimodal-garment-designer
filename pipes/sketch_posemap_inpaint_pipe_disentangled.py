...
class StableDiffusionSketchPosemapInpaintPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        # pose_map_vis = pose_map.sum(dim=1).unsqueeze(1).clamp(0,1)
        # plt.imsave("Pose.jpg", pose_map_vis[0].repeat(3,1,1).permute(1,2,0).cpu().numpy())
        ...
        # plt.imsave("Sketch.jpg", sketch[0].repeat(3,1,1).permute(1,2,0).cpu().numpy())
        sketch = torchvision.transforms.functional.resize(
            sketch, size=(sketch.shape[2] // 8, sketch.shape[3] // 8), 
            interpolation=torchvision.transforms.InterpolationMode.BILINEAR, 
            antialias=True)
        sketch = sketch
        # plt.imsave("Image.png", ((image[0] + 1) / 2).permute(1,2,0).cpu().numpy())
